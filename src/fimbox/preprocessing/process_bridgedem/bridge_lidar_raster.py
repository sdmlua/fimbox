...
def _fetch_ept_points(
    ...
    tiles = _intersecting_tiles(hierarchy, ept_bounds, (qxmin, qymin, qxmax, qymax), min_depth)
    if not tiles:
        return None
        ...
        for dx in range(2):
            for dy in range(2):
                for dz in range(2):
                    ...
                    if ck in hierarchy:
                        _recurse(ck,
                                 bx0 if dx==0 else mx, by0 if dy==0 else my, bz0 if dz==0 else mz,
                                 mx  if dx==0 else bx1, my  if dy==0 else by1, mz  if dz==0 else bz1)
...
class generateBridgeRaster:
    ...
    n_workers    : dask thread workers for bridge-level parallelism (default: all CPUs)
    tile_workers : threads for per-bridge tile downloads (default 8)
    ...
    def _load_bridges(self) -> gpd.GeoDataFrame:
        ...
        gdf = gdf[~gdf["_bridge_id"].isin([str(s) for s in self.skip_ids])].reset_index(drop=True)
        return gdf
    ...
    def _assign_lidar_urls(self, footprints, index) -> gpd.GeoDataFrame:
        joined = gpd.overlay(footprints, index[["url", "count", "geometry"]], how="intersection")
        if joined.empty:
            raise RuntimeError("No LiDAR tiles intersect the bridge footprints.")
        joined = joined.loc[joined.groupby("_bridge_id")["count"].idxmax()].reset_index(drop=True)
        return joined

    def _process_dask(self, footprints: gpd.GeoDataFrame):
        point_dir    = str(self._point_dir)
        tif_dir      = str(self._tif_dir)
        resolution   = self.resolution
        tile_workers = self.tile_workers
        min_depth    = self.min_tile_depth
        threshold    = self.bridge_cls_threshold

        delayed_tasks = [
            dask.delayed(_process_one_bridge)(
                ...
                min_tile_depth=min_depth,
                bridge_cls_threshold=threshold,
            )
        ...
        log.info(f"Completed — {len(results)-failed} ok, {failed} failed")
